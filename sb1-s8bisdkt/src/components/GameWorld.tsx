...
interface GameWorldProps {
  ...
  setMapSelectionPosition: React.Dispatch<React.SetStateAction<{ x: number; y: number }>>;
...
const GameWorld: React.FC<GameWorldProps> = ({ 
  ...
  const [currentAttackHitbox, setCurrentAttackHitbox] = useState<AttackHitbox | null>(null);
  ...
  const lastFarmStonePositionRef = useRef<{ x: number, y: number } | null>(null);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === 'escape') {
        ...
        if (showFullMap) {
          ...
          setPlayer(prev => prev ? { ...prev, isInventoryOpen: false } : prev);
        } else if (player.isStatsWindowOpen) {
          setPlayer(prev => prev ? { ...prev, isStatsWindowOpen: false } : prev);
  ...
  }, [player.controlsDisabled, setPlayer, showFullMap, player.isInventoryOpen, player.isStatsWindowOpen, isSkillTreeOpen]);
  ...
  }, [isOfflineMode, showOfflineUI, offlineFarmTimer, currentMap, player.position.x, player.position.y]);
      ...
      if (!farmTarget.shouldDropNothing) {
        ...
        if (emptySlot) {
          ...
          if (notificationsContainer) {
            ...
            notification.className = 'bg-indigo-900 bg-opacity-90 rounded-lg p-3 border border-indigo-600 flex items-center gap-3 animate-fadeIn pointer-events-auto';
            notification.innerHTML = `
              <img src="${selectedChest.icon}" alt="${selectedChest.name}" class="w-10 h-10" style="image-rendering: pixelated;">
              <div>
                <div class="font-bold text-indigo-200">Found ${selectedChest.name}</div>
                <div class="text-xs text-indigo-300">+${chestAmount} ${selectedChest.name}${chestAmount > 1 ? 's' : ''}</div>
              </div>
            `;
        ...
        if (materialAmount > 0) {
          ...
          if (materialSlot) {
            ...
            if (notificationsContainer) {
              const notification = document.createElement('div');
              notification.id = notificationId;
              notification.className = 'bg-green-900 bg-opacity-90 rounded-lg p-3 border border-green-600 flex items-center gap-3 animate-fadeIn pointer-events-auto';
              notification.innerHTML = `
      ...
      enemies.forEach(enemy => {
        if (enemy.lastDamageDealt && enemy.lastDamageTime) {
          ...
          if (timeSinceLastDamage >= 1000) {
            const newHealth = Math.max(0, player.health - enemy.lastDamageDealt);
  ...
  const updateVisibleChunks = useCallback(() => {
    ...
    const endChunkX = Math.min(Math.floor(mapSize.width / CHUNK_SIZE), Math.ceil(viewportRight / CHUNK_SIZE));
    const startChunkY = Math.max(0, Math.floor(viewportTop / CHUNK_SIZE));
    const endChunkY = Math.min(Math.floor(mapSize.height / CHUNK_SIZE), Math.ceil(viewportBottom / CHUNK_SIZE));
  ...
  const getNextStonePosition = () => {
    const existingStonePositions = enemies
      .filter(enemy => enemy.type === 'metin' || enemy.type === 'stone-of-metin')
  ...
  useEffect(() => {
    if (currentAttackHitbox && player.isAttacking) {
      ...
      setEnemies(prevEnemies => 
        AttackManager.checkEnemyHits(currentAttackHitbox, prevEnemies, playerWithCallback)
      );
  ...
  const handleStoneDeath = (stoneId: string) => {
    setEnemies(prev => {
      ...
      const newEnemies = prev.filter(enemy => 
        enemy.id !== stoneId && enemy.parentMetinId !== stoneId
      );
      ...
      if (currentMap !== 'village' && killedStone) {
        ...
        if (Math.random() < 0.5) {
          ...
          const selectedChest = possibleChests[Math.floor(Math.random() * possibleChests.length)];
      ...
      if (currentMap === 'sohan') {
        const currentMetinCount = newEnemies.filter(e => e.type === 'metin').length;
        const currentStoneOfMetinCount = newEnemies.filter(e => e.type === 'stone-of-metin').length;
        ...
        if (spawnStoneOfMetin && currentStoneOfMetinCount < MAX_STONE_OF_METIN) {
        ...
        const currentStoneMap1Count = newEnemies.filter(e => e.type === 'stone-of-map1').length;
  ...
  const handleTigerMove = (id: string, newPosition: { x: number; y: number }) => {
  ...
  const handleRespawn = useCallback((atHome: boolean) => {
  ...
  const handleDamageDealt = useCallback((damage: number, isCritical: boolean = false) => {
    const newDamageNumber = {
      id: generateId(),
      damage,
      position: { ...player.position },
  ...
  const handleTigerusAttack = useCallback((damage: number) => {
    setPlayer(prev => {
      if (!prev) return prev;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (viewportRef.current) {
      ...
      const worldX = (e.clientX - rect.left) / zoomLevel + cameraPositionRef.current.x;
      const worldY = (e.clientY - rect.top) / zoomLevel + cameraPositionRef.current.y;
      worldMousePositionRef.current = { x: Math.round(worldX), y: Math.round(worldY) };
    }
  }, [zoomLevel]);
  ...
  useEffect(() => {
    ...
    checkForMissingStoneIntervalRef.current = setInterval(() => {
      const farmStoneExists = enemies.some(enemy => enemy.isOfflineFarmTarget);
      
      if (!farmStoneExists && isAutoFarming && lastFarmStonePositionRef.current) {
  ...
  }, [isOfflineMode, showOfflineUI, isAutoFarming, enemies, setPlayer, currentMap]);
  ...
  const handleMapSelection = useCallback((mapName: string) => {
  ...
  const displayChestNotification = (chest, amount) => {
    ...
    if (notificationsContainer) {
      ...
      notification.className = 'bg-indigo-900 bg-opacity-90 rounded-lg p-3 border border-indigo-600 flex items-center gap-3 animate-fadeIn pointer-events-auto';
      notification.innerHTML = `
        <img src="${chest.icon}" alt="${chest.name}" class="w-10 h-10" style="image-rendering: pixelated;">
        <div>
          <div class="font-bold text-indigo-200">Found ${chest.name}</div>
          <div class="text-xs text-indigo-300">+${amount} ${chest.name}${amount > 1 ? 's' : ''}</div>
        </div>
      `;
      notificationsContainer.appendChild(notification);
  ...
  return (
    ...
    >
      {isOfflineMode && showOfflineUI && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-80 px-4 py-2 rounded z-50">
          <div className="text-center">
            <div className="text-yellow-400 font-bold mb-2">Auto-Farming Mode</div>
            {!isAutoFarming ? (
              <div className="text-white">Next stone in: {offlineFarmTimer}s</div>
            ) : (
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[300px] bg-black bg-opacity-50 px-3 py-1 rounded z-50 font-mono text-sm">
        <span className="text-green-400">X: {Math.round(player.position.x)}</span>
        <span className="text-blue-400 ml-2">Y: {Math.round(player.position.y)}</span>
      </div>
      
      <div 
        className="absolute"
        style={{
          transform: `scale(${zoomLevel}) translate(${-cameraPositionRef.current.x}px, ${-cameraPositionRef.current.y}px)`,
          transformOrigin: '0 0',
          width: mapSize.width,
      ...
      >
        ...
        {!isBuildingMode && visibleEnemies.map(enemy => {
          if (enemy.type === 'metin') {
        ...
        {!isBuildingMode && currentAttackHitbox && currentAttackHitbox.type === 'sword' && (
          <SwordAttack 
      ...
      {player.isInventoryOpen && (
        <Inventory
          ...
          setInventory={(newInventory) => setPlayer(prev => prev ? { ...prev, inventory: newInventory } : prev)}
      ...
      {isDead && (
        <DeathScreen player={player} onRespawn={handleRespawn} />
      )}
