...
interface MetinStoneDesertProps {
...
const MetinStoneDesert: React.FC<MetinStoneDesertProps> = ({ position, onSpawnWhiteTiger, onDeath, player }) => {
  const [health, setHealth] = useState(6000 + Math.floor(Math.random() * 201));
  ...
  const MAX_TIGERS = 3;
  const METIN_EXP = 25; // Half the experience of mountain metin
  ...
  useEffect(() => {
    if (health <= 0 && !hasDroppedChest) {
      ...
      if (dropRoll < 40) {
        ...
        selectedItem = GameItems.WARRIORS_CHEST; // Only 15% chance for rare items
      }
    ...
    <>
      ...
      >
        <div 
          ...
          style={{
            backgroundImage: 'url(https://i.imgur.com/WquAPAc.gif)',
...
export default MetinStoneDesert;